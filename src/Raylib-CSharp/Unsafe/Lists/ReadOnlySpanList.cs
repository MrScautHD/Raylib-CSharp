...
public class ReadOnlySpanList<T> where T : unmanaged {
    ...
    public ReadOnlySpanList() {
        this._pointers = new List<(nint, int)>();
    }

    /// <summary>
    /// Adds a read-only span of type T to the SpanList.
    /// </summary>
    ...
    /// <summary>
    /// Removes a read-only span of type T from the SpanList.
    /// </summary>
    /// <param name="span">The read-only span of type T to remove.</param>
    public unsafe void Remove(ReadOnlySpan<T> span) {
        fixed (T* ptr = span) {
    ...
    /// <summary>
    /// Determines whether the SpanList contains the specified read-only span of type T.
    /// </summary>
    /// <param name="span">The read-only span of type T to locate in the SpanList.</param>
    /// <returns>
    /// true if the SpanList contains the specified span; otherwise, false.
    /// </returns>
    public unsafe bool Contains(ReadOnlySpan<T> span) {
        fixed (T* ptr = span) {
    ...
    /// </returns>
    public unsafe int IndexOf(ReadOnlySpan<T> span) {
        fixed (T* ptr = span) {
    ...
    /// <summary>
    /// Inserts a span of type T at the specified index in the ReadOnlySpanList.
    /// </summary>
    ...
    public unsafe void Insert(int index, ReadOnlySpan<T> span) {
        fixed (T* ptr = span) {
    ...
    /// <summary>
    /// Indexer for accessing the elements in the ReadOnlySpanList by index.
    /// </summary>
